...
export default function EditorTabs() {
  ...
  return (
    <div className="flex shrink-0 items-stretch bg-editor-soft text-[13px]">
      <div className="flex min-w-0 flex-1 items-stretch overflow-x-auto border-b border-chrome-strong">
        {TABS.map((tab) => {
          ...
          return (
            <Link
              key={tab.path}
              href={tab.path}
              className={`group relative flex shrink-0 items-center gap-2 border-r border-chrome px-3 py-2 transition-colors md:px-4 ${
                active
